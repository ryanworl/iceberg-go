...
func newRowDeltaV3CommitTestTable(t *testing.T) (*table.Table, string) {
	t.Helper()
	...
	return table.New(
		table.Identifier{"db", "row_delta_dv_test"},
		meta, location+"/metadata/v1.metadata.json",
		func(ctx context.Context) (iceio.IO, error) {
			return iceio.LocalFS{}, nil
		},
...
func writeDV(t *testing.T, location, name, dataPath string, positions []int64) iceberg.DataFile {
	...
	w := dv.NewDVWriter(iceio.LocalFS{})
...
func newTableWithLiveDV(t *testing.T) (*table.Table, string, string, iceberg.DataFile) {
	...
	tbl, location := newRowDeltaV3CommitTestTable(t)
...
// deletion-vector file paths. Row visibility is asserted at the plan
// level because the Arrow record path still rejects DV tasks with
// ErrNotImplemented.
...
// DV with status DELETED, and the scanner plans exactly the replacement
// DV, whose contents hide positions 1 and 3.
func TestRowDeltaDVSupersessionSingleSnapshot(t *testing.T) {
	...
	// Row visibility: the scanner plans exactly the replacement DV, and
	// its bitmap hides positions 1 and 3 — ids 1/3/5 remain visible.
...
func TestRowDeltaRemoveDeletesUnknownFile(t *testing.T) {
	...
	t.Run("no current snapshot", func(t *testing.T) {
		tbl, _ := newRowDeltaV3CommitTestTable(t)
		rd := tbl.NewTransaction().NewRowDelta(nil).
			AddDeletes(newDV).
			RemoveDeletes(oldDV)
