...
func (b *MetadataBuilder) AddSortOrder(sortOrder *SortOrder) error {
	...
	if _, err := b.GetSortOrderByID(newOrderID); err == nil {
		...
		// do NOT emit an add-sort-order update (catalogs reject adding
		// a sort order under an existing ID). Record it as last-added
		// so a following SetDefaultSortOrderID(-1) resolves to the
		// reused ID; SetDefaultSortOrderID encodes the concrete ID in
		// that case because no add update precedes it.
