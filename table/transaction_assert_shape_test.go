...
func withDefaultSpecChanged(t *testing.T, base Metadata) Metadata {
	...
	spec := iceberg.NewPartitionSpecID(base.DefaultPartitionSpec()+1,
		iceberg.PartitionField{SourceIDs: []int{1}, FieldID: iceberg.PartitionDataIDStart,
...
// own (e.g. a properties-only revision marker) commits without any
...
// both defaults at their base values.
...
func TestTransactionAssertDefaultShape(t *testing.T) {
	...
	t.Run("succeeds when the shape is unmoved", func(t *testing.T) {
		...
		require.NoError(t, tx.SetProperties(iceberg.Properties{"spec-rev": "7"}))

		_, err := tx.Commit(t.Context())
		require.NoError(t, err)
		assert.Equal(t, "7", cat.metadata.Properties()["spec-rev"])
	})
	...
	t.Run("fails when the default spec moved", func(t *testing.T) {
		...
		assert.NotContains(t, cat.metadata.Properties(), "spec-rev",
