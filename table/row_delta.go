...
//
// Only deletion vectors (delete files with a referenced data file) can
// be removed here. Expunging fully-applied position or equality delete
// files belongs to the rewrite path ([Transaction.ReplaceFiles] /
// [RewriteFiles]), which owns the data-file replacement validation
// that makes such removals safe.
//
...
func (rd *RowDelta) Commit(ctx context.Context) error {
	...
	if len(rd.removedDels) > 0 {
		...
		resolvedRemovals, err := rd.resolveRemovedDeletes(fs)
		if err != nil {
			return err
		}
		...
		for _, live := range resolvedRemovals {
			producer.removeDeleteFile(live)
		}
	} else {
...
//
//   - every removed entry must be a deletion vector (a pos-delete file
//     with referenced_data_file set); other delete files are removed
//     through the rewrite path, which validates them differently.
...
func (rd *RowDelta) validateRemovedDeletes(resolved []iceberg.DataFile) error {
	...
	for _, live := range resolved {
		ref := live.ReferencedDataFile()
		if live.ContentType() != iceberg.EntryContentPosDeletes || ref == nil || *ref == "" {
			return fmt.Errorf("only deletion vectors can be removed by a row delta, got content type %s: %s",
				live.ContentType(), live.FilePath())
		}
...
// the commit would strand two live DVs on one data file.
func (rd *RowDelta) resolveRemovedDeletes(fs iceio.IO) ([]iceberg.DataFile, error) {
	snap := rd.txn.meta.currentSnapshot()
	if snap == nil {
	...
	seen := make(map[removalKey]struct{}, len(rd.removedDels))
	...
	for i, f := range rd.removedDels {
		...
		default:
			...
			for _, c := range candidates {
				if cr := c.ReferencedDataFile(); cr != nil && *cr == *ref {
					live = c
		...
		key := removalKey{path: live.FilePath(), ref: referencedDataFile(live)}
		if _, ok := seen[key]; ok {
